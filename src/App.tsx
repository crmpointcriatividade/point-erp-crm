...
import {
  ...
  ArrowRight, AlertTriangle, RefreshCw, Trash2, Menu,
  ChevronLeft, LogOut, Shield, UserCheck, Eye, EyeOff,
} from 'lucide-react';
...
type ModalType = 'pedido' | 'cliente' | 'fornecedor' | 'compra' | 'novoProduto' | 'novoClienteRapido' | null;
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-600 to-indigo-900 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-sm">
        ...
        <div className="space-y-4">
          <Campo label="Usuário"><input type="text" placeholder="Seu nome de usuário" value={nome} onChange={e => setNome(e.target.value)} onKeyDown={e => e.key==='Enter'&&login()} className={inputClass}/></Campo>
          <Campo label="Senha">
          ...
          <button onClick={login} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold text-sm shadow-lg shadow-indigo-200 transition-all active:scale-95">Entrar</button>
        </div>
        <div className="mt-6 p-4 bg-slate-50 rounded-2xl space-y-2">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Usuários de teste</p>
          <div className="flex items-center gap-2 text-xs text-slate-500"><Shield size={12} className="text-indigo-500"/><span><b>Admin Point</b> / admin123</span></div>
          <div className="flex items-center gap-2 text-xs text-slate-500"><UserCheck size={12} className="text-emerald-500"/><span><b>Colaborador</b> / colab123</span></div>
        </div>
      </motion.div>
    </div>
  );
...
export default function App() {
  ...
  const tabs = [
    ...
    { id: 'vendas', label: 'Vendas', icon: <ShoppingCart size={20}/>, roles: ['admin'] },
  ...
  const headerBtn = () => {
    ...
    if (activeTab==='fornecedores'&&isAdmin) return { label:'Novo Fornecedor', action:()=>setModal('fornecedor') };
        ...
        <AnimatePresence>
          {sidebarOpen && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              className="fixed inset-0 bg-slate-900/50 z-30 lg:hidden" onClick={()=>setSidebarOpen(false)}/>
          )}
        </AnimatePresence>

        {/* SIDEBAR */}
        <aside className={cn(
          'fixed lg:relative inset-y-0 left-0 z-40 flex flex-col bg-white border-r border-slate-200 transition-transform duration-300 shrink-0 w-64',
          ...
          <div className="p-4 border-t border-slate-100">
            <div className="flex items-center gap-3 p-2 rounded-xl">
              ...
              <button onClick={()=>setUser(null)} title="Sair" className="p-1.5 text-slate-300 hover:text-rose-500 rounded-lg transition-colors"><LogOut size={16}/></button>
            </div>
          </div>
        </aside>
...
function Toast({message,onClose}:{message:string;onClose:()=>void}) {
  ...
  return <motion.div initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} exit={{opacity:0,y:40}} className="fixed bottom-6 right-6 z-[100] bg-emerald-600 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 font-bold text-sm"><CheckCircle2 size={20}/>{message}</motion.div>;
}
...
function ConfigView() {
  return (
    <div className="space-y-6 max-w-2xl">
      <div><h2 className="text-2xl md:text-3xl font-black text-slate-900">Configurações</h2><p className="text-slate-500 text-sm">Acesso e permissões</p></div>
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100"><h3 className="font-black text-slate-800">Controle de Acesso</h3></div>
        <div className="divide-y divide-slate-100">
          {[
            { role:'Administrador', tag:'admin', icon:<Shield size={22} className="text-indigo-600"/>, bg:'bg-indigo-50', tbg:'bg-indigo-100 text-indigo-600', perms:['CRM/Kanban','Insumos','Clientes','Fornecedores','Vendas','Compras','Configurações'], user:'Admin Point', senha:'admin123' },
            { role:'Colaborador', tag:'colaborador', icon:<UserCheck size={22} className="text-emerald-600"/>, bg:'bg-emerald-50', tbg:'bg-emerald-100 text-emerald-600', perms:['CRM/Kanban','Clientes','Compras'], user:'Colaborador', senha:'colab123' },
          ].map(r=>(
            <div key={r.tag} className="p-6 flex items-start gap-4">
              <div className={cn('p-3 rounded-2xl shrink-0',r.bg)}>{r.icon}</div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <p className="font-black text-slate-800">{r.role}</p>
                  <span className={cn('text-[10px] font-black uppercase px-2 py-0.5 rounded-full',r.tbg)}>{r.tag}</span>
                </div>
                <div className="flex flex-wrap gap-2 mb-2">{r.perms.map(p=><span key={p} className="text-xs bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full font-medium">{p}</span>)}</div>
                <p className="text-xs text-slate-400 font-mono">Login: <b>{r.user}</b> / Senha: <b>{r.senha}</b></p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3">
        ...
        <p className="text-sm text-amber-700">Para usuários com e-mail/senha próprios, ative o <b>Supabase Auth</b> e substitua a lista <code className="bg-amber-100 px-1 rounded">USUARIOS</code> no <code className="bg-amber-100 px-1 rounded">App.tsx</code>.</p>
      </div>
    </div>
  );
}
...
function KanbanView({searchQuery,onNovoPedido}:{searchQuery:string;onNovoPedido:()=>void}) {
  ...
  const zap=(pedido:Pedido)=>{const n=(pedido.clientes?.whatsapp||pedido.cliente_contato_avulso||'').replace(/\D/g,'');const m=encodeURIComponent(`Olá! Seu orçamento *#${pedido.codigo}* está pronto. Total: R$ ${Number(pedido.valor_total).toFixed(2)}. Confirma?`);if(n)window.open(`https://wa.me/55${n}?text=${m}`,'_blank');};
    ...
    <div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4" style={{minHeight:'calc(100vh - 240px)'}}>
        {statuses.map(status=>{
          ...
          const cc=STATUS_COLORS[status.nome]||'bg-slate-100 border-slate-200 text-slate-600';
            ...
            <div key={status.id} className="w-72 md:w-80 flex-shrink-0 flex flex-col">
              ...
              <div className="flex-1 bg-slate-100/40 p-3 space-y-3 rounded-b-2xl border border-slate-200 border-t-0 overflow-y-auto">
                ...
                {col.map(p=>{
                  ...
                  const mov=movendo===p.id;
                  return(
                    <motion.div key={p.id} layout className={cn('bg-white p-4 rounded-2xl shadow-sm border border-slate-200 hover:border-indigo-400 transition-all group',mov&&'opacity-50 pointer-events-none')}>
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[10px] font-black text-slate-300 uppercase">#{p.codigo}</span>
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button onClick={()=>generateBudgetPDF(p,[])} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><FileText size={13}/></button>
                          <button onClick={()=>zap(p)} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg"><MessageSquare size={13}/></button>
                        </div>
                      </div>
                      <h4 className="font-bold text-slate-800 text-sm leading-tight mb-1">{nome}</h4>
                      ...
                      <div className="flex justify-between items-center pt-3 border-t border-slate-50">
                        ...
                        {next&&<button onClick={()=>mover(p,next.nome)} disabled={mov} className="flex items-center gap-1 text-[10px] font-black text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded-lg uppercase">{next.nome}<ArrowRight size={9}/></button>}
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-x-auto">
        <table className="w-full text-left min-w-[650px]">
          ...
          <tbody className="divide-y divide-slate-100">
            {insumos.length===0&&<tr><td colSpan={7} className="px-5 py-10 text-center text-slate-400">Nenhum insumo encontrado.</td></tr>}
...
function ComprasView({searchQuery,onAdd}:{searchQuery:string;onAdd:()=>void}) {
  ...
  useEffect(()=>{supabase.from('compras').select('*').order('created_at',{ascending:false}).then(({data})=>{setCompras(data||[]);setLoading(false);}).catch(()=>setLoading(false));},[]);
  ...
  return(
    <div className="space-y-5">
      <div className="flex justify-between items-end flex-wrap gap-3">
        <div><h2 className="text-2xl md:text-3xl font-black">Compras</h2><p className="text-slate-500 text-sm">Registro de compras de insumos</p></div>
      ...
      {f.length===0?(
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-x-auto">
          <table className="w-full text-left min-w-[500px]">
            <thead><tr className="bg-slate-50 border-b border-slate-100">{['Fornecedor','Data','Nota Fiscal','Total','Status'].map(h=><th key={h} className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-slate-100">
              {f.map(c=>(
                <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-bold text-slate-800 text-sm">{c.fornecedor_nome}</td>
                  <td className="px-6 py-4 text-sm text-slate-500">{c.data?new Date(c.data).toLocaleDateString('pt-BR'):'—'}</td>
                  <td className="px-6 py-4 text-sm text-slate-500">{c.nota_fiscal||'—'}</td>
                  <td className="px-6 py-4 font-black text-indigo-600 text-sm">R$ {Number(c.total||0).toFixed(2)}</td>
                  <td className="px-6 py-4"><span className="text-[10px] font-black uppercase bg-emerald-50 text-emerald-600 px-2 py-1 rounded-full">{c.status||'Recebido'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
...
interface ItemOrc{id:string;produto_id:string;produto_nome:string;descricao:string;quantidade:number;valor_unitario:number;}
function ModalNovoPedido({onClose,onAbrirNovoProduto,onAbrirNovoCliente}:{onClose:()=>void;onAbrirNovoProduto:()=>void;onAbrirNovoCliente:()=>void;}) {
  ...
  const clisFiltrados=clientes.filter(c=>c.nome.toLowerCase().includes(buscaCli.toLowerCase())&&buscaCli.length>0).slice(0,6);
  ...
  const addItem=(p:Produto)=>{setItens(prev=>[...prev,{id:crypto.randomUUID(),produto_id:p.id,produto_nome:p.nome,descricao:p.descricao||p.nome,quantidade:1,valor_unitario:0}]);setBuscaProd('');setShowProd(false);};
  const addManual=()=>{setItens(prev=>[...prev,{id:crypto.randomUUID(),produto_id:'',produto_nome:'',descricao:buscaProd||'',quantidade:1,valor_unitario:0}]);setBuscaProd('');setShowProd(false);};
  ...
  const salvar=async()=>{
    const nome=cli?.nome||buscaCli.trim();if(!nome){setErro('Informe o cliente.');return;}
    if(!statusInicial){setErro('Aguarde...');return;}
    setSalvando(true);
    const{data:pedido,error}=await criarPedido({cliente_id:cli?.id||undefined,cliente_nome_avulso:cli?undefined:nome,cliente_contato_avulso:cli?.whatsapp||undefined,status_id:statusInicial.id,data_entrega:form.data_entrega||undefined,observacoes:form.observacoes||undefined});
    if(error||!pedido){setErro('Erro: '+(error?.message||''));setSalvando(false);return;}
    const linhas=itens.filter(i=>i.descricao.trim()||i.produto_id);
    if(linhas.length>0)await supabase.from('itens_pedido').insert(linhas.map(i=>({pedido_id:(pedido as any).id,produto_id:i.produto_id||null,quantidade:i.quantidade,preco_unitario:i.valor_unitario,descricao_custom:i.descricao})));
    setToast('Orçamento criado com sucesso!');setTimeout(onClose,1800);
  };
  return(
    <>
    <ModalWrapper title="Novo Orçamento" onClose={onClose} size="lg">
      ...
      <div className="bg-slate-50 rounded-2xl p-4 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onAbrirNovoCliente} className="flex items-center gap-1 text-xs font-bold text-indigo-600 hover:underline"><Plus size={11}/>Cadastrar novo cliente</button>
        </div>
        <div className="relative" ref={cliRef}>
          <input type="text" placeholder="Buscar cliente ou digitar nome avulso..." className={inputClass}
            value={cli?cli.nome:buscaCli} onChange={e=>{setBuscaCli(e.target.value);setCli(null);setShowCli(true);}} onFocus={()=>setShowCli(true)}/>
          ...
          {showCli&&clisFiltrados.length>0&&(
            <div className="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden max-h-48 overflow-y-auto">
              {clisFiltrados.map(c=>(
                <button key={c.id} onClick={()=>{setCli(c);setBuscaCli('');setShowCli(false);}} className="w-full text-left px-4 py-3 hover:bg-indigo-50 transition-colors flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-black text-xs shrink-0">{c.nome.charAt(0)}</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Campo label="Observações"><input type="text" placeholder="Arte, cores, acabamento..." value={form.observacoes} onChange={e=>setForm({...form,observacoes:e.target.value})} className={inputClass}/></Campo>
      </div>
      {/* ITENS */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-xs font-black text-slate-500 uppercase tracking-wider">Itens do Orçamento</p>
          <button onClick={onAbrirNovoProduto} className="flex items-center gap-1 text-xs font-bold text-indigo-600 hover:underline"><Plus size={11}/>Cadastrar novo produto</button>
        </div>
              ...
              {prodsFiltrados.length>0?(
                <>
                  {prodsFiltrados.map(p=>(
                    <button key={p.id} onClick={()=>addItem(p)} className="w-full text-left px-4 py-3 hover:bg-indigo-50 transition-colors flex items-center justify-between">
                  ...
                  <button onClick={addManual} className="w-full text-left px-4 py-3 border-t border-slate-100 hover:bg-slate-50 transition-colors flex items-center gap-2 text-sm text-slate-500 font-medium"><Plus size={13} className="text-slate-400"/>Adicionar "{buscaProd}" manualmente</button>
                </>
              ):(
                <div className="p-4"><p className="text-sm text-slate-500 mb-2">Produto não encontrado.</p><button onClick={addManual} className="flex items-center gap-2 text-sm text-indigo-600 font-bold hover:underline"><Plus size={13}/>Adicionar "{buscaProd}" como item manual</button></div>
              )}
            </div>
          )}
        </div>
        {itens.length>0?(
          <div className="border border-slate-200 rounded-2xl overflow-hidden overflow-x-auto">
            <table className="w-full text-sm min-w-[460px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {itens.map(item=>(
                  <tr key={item.id}>
                    <td className="px-3 py-2"><input type="text" value={item.descricao} onChange={e=>upd(item.id,'descricao',e.target.value)} placeholder="Descrição" className="w-full text-sm bg-transparent border-b border-transparent focus:border-indigo-400 outline-none py-0.5"/></td>
                    ...
                    <td className="px-3 py-2"><button onClick={()=>del(item.id)} className="text-slate-300 hover:text-rose-500 transition-colors"><Trash2 size={13}/></button></td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center text-slate-400 text-sm font-medium">Busque um produto acima ou adicione manualmente</div>
        )}
      </div>
...
function ModalNovoProduto({onClose}:{onClose:()=>void}) {
  ...
  const salvar=async()=>{if(!form.nome.trim()){setErro('Nome obrigatório.');return;}setSalvando(true);const{error}=await supabase.from('produtos').insert({nome:form.nome,descricao:form.descricao||null,categoria:form.categoria,markup_sugerido:Number(form.markup)||2.5,custo_mao_obra_hora:Number(form.mao)||25,ativo:true});if(error){setErro('Erro: '+error.message);setSalvando(false);}else{setToast('Produto cadastrado! Volte ao orçamento para buscá-lo.');setTimeout(onClose,2000);}};
  return(
    <>
    <ModalWrapper title="Cadastrar Produto" onClose={onClose}>
      {erro&&<MsgErro msg={erro}/>}
      <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-sm text-indigo-700 font-medium">💡 Após cadastrar, feche e busque o produto no orçamento.</div>
      <Campo label="Nome *"><input type="text" placeholder="Ex: Kit Festa Safari" value={form.nome} onChange={e=>setForm({...form,nome:e.target.value})} className={inputClass}/></Campo>
      <Campo label="Descrição"><input type="text" placeholder="Descrição breve" value={form.descricao} onChange={e=>setForm({...form,descricao:e.target.value})} className={inputClass}/></Campo>
      <div className="grid grid-cols-3 gap-3">
        <Campo label="Categoria"><select value={form.categoria} onChange={e=>setForm({...form,categoria:e.target.value})} className={inputClass}><option value="kit">Kit</option><option value="adesivo">Adesivo</option><option value="impresso">Impresso</option><option value="personalizado">Personalizado</option></select></Campo>
...
function ModalNovoCliente({onClose}:{onClose:()=>void}) {
  ...
  const[salvando,setSalvando]=useState(false);const[erro,setErro]=useState('');const[toast,setToast]=useState('');
  const salvar=async()=>{if(!form.nome.trim()){setErro('Nome obrigatório.');return;}setSalvando(true);const{error}=await criarCliente(form);if(error){setErro('Erro: '+error.message);setSalvando(false);}else{setToast('Cliente salvo com sucesso! ✅');setTimeout(onClose,1800);}};
  return(
    <>
    <ModalWrapper title="Novo Cliente" onClose={onClose}>
      {erro&&<MsgErro msg={erro}/>}
      <Campo label="Nome *"><input type="text" placeholder="Ex: Maria Silva" value={form.nome} onChange={e=>setForm({...form,nome:e.target.value})} className={inputClass}/></Campo>
      ...
      <BotaoSalvar onClick={salvar} loading={salvando} label="Salvar Cliente"/>
    </ModalWrapper>
    <AnimatePresence>{toast&&<Toast message={toast} onClose={()=>setToast('')}/>}</AnimatePresence>
    </>
  );
...
function ModalNovoFornecedor({onClose}:{onClose:()=>void}) {
  ...
  return(
    ...
    <ModalWrapper title="Novo Fornecedor" onClose={onClose}>
      {erro&&<MsgErro msg={erro}/>}
      <Campo label="Nome *"><input type="text" placeholder="Ex: Papéis & Cia" value={form.nome} onChange={e=>setForm({...form,nome:e.target.value})} className={inputClass}/></Campo>
      <div className="grid grid-cols-2 gap-4">
        ...
        <Campo label="Contato"><input type="text" placeholder="Ricardo" value={form.contato} onChange={e=>setForm({...form,contato:e.target.value})} className={inputClass}/></Campo>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <Campo label="WhatsApp"><input type="text" placeholder="(11) 99999-0000" value={form.whatsapp} onChange={e=>setForm({...form,whatsapp:e.target.value})} className={inputClass}/></Campo>
        <Campo label="E-mail"><input type="email" placeholder="email@empresa.com" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} className={inputClass}/></Campo>
      </div>
      <Campo label="Cidade"><input type="text" placeholder="São Paulo" value={form.cidade} onChange={e=>setForm({...form,cidade:e.target.value})} className={inputClass}/></Campo>
      <BotaoSalvar onClick={salvar} loading={salvando} label="Salvar Fornecedor"/>
    </ModalWrapper>
    <AnimatePresence>{toast&&<Toast message={toast} onClose={()=>setToast('')}/>}</AnimatePresence>
    </>
  );
...
function ModalNovaCompra({onClose}:{onClose:()=>void}) {
  ...
  const[buscaF,setBuscaF]=useState('');const[forn,setForn]=useState<any|null>(null);const[showF,setShowF]=useState(false);
  ...
  const[salvando,setSalvando]=useState(false);const[erro,setErro]=useState('');const[toast,setToast]=useState('');
  const fornF=fornecedores.filter(f=>f.nome.toLowerCase().includes(buscaF.toLowerCase())&&buscaF.length>0).slice(0,6);
  const total=itens.reduce((a,i)=>a+i.quantidade*i.valor_unitario,0);
  ...
  return(
    ...
    <ModalWrapper title="Nova Compra" onClose={onClose} size="lg">
      ...
      <div className="bg-slate-50 rounded-2xl p-4 space-y-2">
        ...
        <div className="relative">
          <input type="text" placeholder="Buscar fornecedor..." className={inputClass} value={forn?forn.nome:buscaF} onChange={e=>{setBuscaF(e.target.value);setForn(null);setShowF(true);}} onFocus={()=>setShowF(true)}/>
          ...
          {showF&&fornF.length>0&&<div className="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden">{fornF.map(f=><button key={f.id} onClick={()=>{setForn(f);setBuscaF('');setShowF(false);}} className="w-full text-left px-4 py-3 hover:bg-indigo-50 transition-colors"><p className="font-bold text-sm text-slate-800">{f.nome}</p><p className="text-xs text-slate-400">{f.cnpj||'—'}</p></button>)}</div>}
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <button onClick={addL} className="text-xs font-bold text-indigo-600 hover:underline flex items-center gap-1"><Plus size={11}/>Adicionar linha</button>
        </div>
        <div className="border border-slate-200 rounded-2xl overflow-hidden overflow-x-auto">
      ...
      <Campo label="Observações"><input type="text" placeholder="Material para estoque de fevereiro" value={form.observacoes} onChange={e=>setForm({...form,observacoes:e.target.value})} className={inputClass}/></Campo>
  ...
  return(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
      <motion.div initial={{scale:0.95,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.95,opacity:0}} transition={{duration:0.15}}
        className={cn('bg-white rounded-3xl shadow-2xl overflow-hidden w-full',size==='lg'?'max-w-2xl':'max-w-lg')}>
...
function PlaceholderView({title,icon}:{title:string;icon:React.ReactNode}) {
  return <div className="h-[60vh] flex flex-col items-center justify-center text-slate-300 space-y-4"><div className="p-8 bg-slate-100 rounded-full text-slate-200">{icon}</div><h2 className="text-2xl font-black text-slate-400 uppercase tracking-widest">{title}</h2><p className="text-slate-400 font-medium text-sm">Em desenvolvimento.</p></div>;
}

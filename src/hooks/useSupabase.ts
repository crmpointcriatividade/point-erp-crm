...
export function usePedidos(search = '') {
  ...
  const fetch = useCallback(async () => {
    ...
    if (search.trim()) {
      query = query.or(
        `cliente_nome_avulso.ilike.%${search}%,codigo.ilike.%${search}%`
      );
    }
  ...
  const baixarEstoque = async (pedidoId: string) => {
    return supabase.rpc('baixar_estoque_pedido', { p_pedido_id: pedidoId });
  };
...
// ============================================================
// Hook: Insumos com alerta de estoque baixo
// ============================================================
